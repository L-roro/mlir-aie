...
def my_matmul():
    ...
    A_sz_in_i32s = int(M * K * word_size_in // 4)
    B_sz_in_i32s = int(K * N * word_size_in // 4)
    C_sz_in_bytes = int(M * N * word_size_out)
    ...
    k_in_i32s = int(k * word_size_in // 4)
    K_in_i32s = int(K * word_size_in // 4)

    # Matrix B: KxN, submatrices b: kxn
    n_in_i32s = int(n * word_size_in // 4)
    N_in_i32s = int(N * word_size_in // 4)
    k_x_N_in_i32s = int(k * N * word_size_in // 4)

    # Output Matrix C: MxN
    n_in_i32s_out = int(n * word_size_out // 4)
    N_in_i32s_out = int(N * word_size_out // 4)
    m_x_N_in_i32s_out = int(m * N * word_size_out // 4)

    with mlir_mod_ctx() as ctx:

        @device(AIEDevice.npu2)
        def device_body():
            memref_a_ty = T.memref(int(m*1.125), k, T.i8()) # we don't have T.bfp16() yet - Check
            memref_b_ty = T.memref(int(k*1.125), n, T.i8()) # we don't have T.bfp16() yet
            memref_c_ty = T.memref(int(m*1.125), n, T.i8()) # we don't have T.bfp16() yet
            ...
            zero_scalar = external_func("zero_scalar_i8", inputs=[memref_c_ty])
            zero = external_func("zero_bfp16", inputs=[memref_c_ty])
            matmul_scalar = external_func(
                "matmul_scalar_i8_i8",
                inputs=[memref_a_ty, memref_b_ty, memref_c_ty],
            )
            matmul = external_func(
                "matmul_bfp16_bfp16", inputs=[memref_a_ty, memref_b_ty, memref_c_ty]
            )
            ...
            def sequence(A, B, C):
                ...
                for tile_row_block in range(
                    ...
                    C_row_offset_in_i32s = (
                        int(tile_row_block * rows_per_block * m * N * word_size_out // 4)
                    )
                    num_tile_rows = min(
                    ...
                    for tile_row in range(num_tile_rows):
                        A_row_offset_in_i32s = (
                            int(((tile_row_block * rows_per_block) + tile_row)
                            * m
                            * K
                            * word_size_in
                            // 4)
