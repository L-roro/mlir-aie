...
void matmul_testing_kernel(bfp16ebs8 *__restrict pA, bfp16ebs8 *__restrict pB,
  ...
  // matmul_vectorized_2x2_bfp16_bfp16<32, 32, 32, 8, 8, 8>((int8 *)pA, (int8
  // *)pB,
  //                                                        (int8 *)pC);
  // aie::block_vector_input_buffer_stream<bfp16ebs8, 64> inStreamA(
  //     (bfp16ebs8 *)pA);
  // aie::bfp_vector_output_buffer_stream<bfp16ebs8, 64> outStreamC(
  //     (bfp16ebs8 *)pC);
  // for (int i = 0; i < 32 * 32 / 64; i++) {
  //   outStreamC.push(inStreamA.pop());
  // }
  // Test writing constant values to see if I can debug what is going on.
  aie::block_vector_output_buffer_stream<bfp16ebs8, 64> outStreamC(pC);

  aie::vector<bfloat16, 64> data = aie::broadcast<bfloat16, 64>(1);
  aie::accum<accfloat, 64> acc(data);

  aie::vector<bfloat16, 64> data2 = aie::broadcast<bfloat16, 64>(-1);
  aie::accum<accfloat, 64> acc2(data2);

  outStreamC << acc.to_vector<bfp16ebs8>();
  outStreamC << acc2.to_vector<bfp16ebs8>();

  int8_t *p = (int8_t *)pC;
  int8_t *pAt = (int8_t *)pA;
  for (int i = 0; i < 32 * 32 * 1.125; i++) {
    p[i] = pAt[i];
    // p[i] = i / 9;
  }
}

// For fuck's sake be careful, if you overwrite while writing the zeros in c,
// you might end up modifying the part of a or b that is in the same memory
// bank... Very hard to debug!
