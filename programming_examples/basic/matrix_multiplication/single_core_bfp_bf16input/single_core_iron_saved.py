...
from aie.dialects.aiex import bfp16ebs8, bfp16ebs16

dtype_map = {
    "bf16": bfloat16,
    "f32": np.float32,
    "bfp16ebs8": bfp16ebs8,
    "bfp16ebs16": bfp16ebs16,
}
...
def main():
    ...
    if args.generate_taps:
        ...
        maybe_module[1].visualize(file_path="col-maj-1-B.png")
        return maybe_module
    else:
        # print mlir
        print(maybe_module)
...
def my_matmul(
    ...
    # These are conditions that must be satisfied on all datatypes
    assert M % m == 0
    assert K % k == 0
    assert N % n == 0

    # Supported instrincsics for bfp16 types are 8x8_8x8T and 4x8_16x8T
    r = 8
    s = 8
    t = 8
    # r = 4
    ...
    # Define tensor types
    # These are the full tensor types
    A_ty = np.ndarray[(M * K,), np.dtype[dtype_in]]
    B_ty = np.ndarray[(K * N,), np.dtype[dtype_in]]
    C_ty = np.ndarray[(M * N,), np.dtype[dtype_out]]
    # These are the tile types
    a_ty = np.ndarray[(m, k), np.dtype[dtype_in]]
    b_ty = np.ndarray[(k, n), np.dtype[dtype_in]]
    c_ty = np.ndarray[(m, n), np.dtype[dtype_out]]

    # AIE Core Function declarations
    # TODO: Choose an appropriate kernel name once it is written
    ...
    matmul_vectorized_func_name = "matmul_testing_kernel2"
    ...
    # TODO: Ask someone about this - How has this been chosen?
    rows_per_block = 4
        ...
        for tile_row_block in range(ceildiv(M_div_m, rows_per_block)):
            ...
            for pingpong in [0, 1]:
                ...
                if num_tile_rows <= 0:
                    ...
                    break
                # Remember that task groups represent a synchronized unit of work, which means that
                # all operations in a task group are performed simultaneously and waited on before starting the next group.
    ...
    if dev == "npu":
        print(
            "Block datatypes are unsupported on the NPU1 device, cannot create program"
        )
        return
    else:
        dev_ty = NPU2()
